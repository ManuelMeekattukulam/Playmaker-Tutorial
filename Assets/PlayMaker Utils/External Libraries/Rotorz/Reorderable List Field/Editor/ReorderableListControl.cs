...
namespace HutongGames.Rotorz.ReorderableList {
	...
	public class ReorderableListControl {
		...
		private void PrepareState(int controlID, IReorderableListAdaptor adaptor) {
			...
			_visibleRect = GUIHelperPlayMaker.VisibleRect();
		...
		private bool DoRemoveButton(Rect position, bool visible) {
			...
			return GUIHelperPlayMaker.IconButton(position, visible, iconNormal, iconActive, ItemButtonStyle);
		}
			...
			try {
				...
				if (isRepainting && isVisible) {
					...
					if (isDraggable) {
						...
						GUIHelperPlayMaker.DrawTexture(texturePosition, ReorderableListResources.GetTexture(ReorderableListTexture.GrabHandle));
					}
					...
					if (!_tracking || itemIndex != s_AnchorIndex) {
						if (itemIndex != 0 || HorizontalLineAtStart) {
							...
							GUIHelperPlayMaker.Separator(horizontalLinePosition, HorizontalLineColor);
						}
					}
				}
		...
		private void DrawFloatingListItem(IReorderableListAdaptor adaptor, float targetSlotPosition) {
			if (Event.current.type == EventType.Repaint) {
				...
				GUIHelperPlayMaker.Separator(targetPosition, HorizontalLineColor);
				...
				GUIHelperPlayMaker.DrawTexture(targetPosition, EditorGUIUtility.whiteTexture);
				...
				GUIHelperPlayMaker.DrawTexture(s_DragItemPosition, EditorGUIUtility.whiteTexture);
...
//*/
		...
		private void AutoScrollTowardsMouse() {
			...
			Rect visiblePosition = GUIHelperPlayMaker.VisibleRect();
			Vector2 mousePosition = Event.current.mousePosition;
		...
		protected virtual void DrawDropIndicator(Rect position) {
			GUIHelperPlayMaker.Separator(position);
		}
		...
		private void CheckForAutoFocusControl() {
			...
			if (s_AutoFocusControlID == _controlID) {
				...
				GUIHelperPlayMaker.FocusTextInControl("AutoFocus_" + _controlID + "_" + s_AutoFocusIndex);
				s_AutoFocusIndex = -1;
			}
		}
		...
		private void DrawFooterControls(Rect position, IReorderableListAdaptor adaptor) {
			if (HasFooterControls) {
				...
				if (HasAddButton) {
					...
					if (GUIHelperPlayMaker.IconButton(buttonPosition, true, iconNormal, iconActive, FooterButtonStyle)) {
						// Append item to list.
						GUIUtility.keyboardControl = 0;
						AddItem(adaptor);
					}
				...
				if (HasAddMenuButton) {
					// Draw add menu drop-down button.
					if (GUIHelperPlayMaker.IconButton(menuButtonPosition, true, menuIconNormal, menuIconActive, FooterButtonStyle)) {
						GUIUtility.keyboardControl = 0;
